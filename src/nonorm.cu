...
#pragma omp parallel for
  ...
  musaMemcpyAsync(pimpl->dev.padded_templates.get(), padded_templates.get(),
                  sizeof(cufftComplex) * padded_template_size * batch_size, musaMemcpyHostToDevice,
                  pimpl->dev.stream);
...
void SearchNoNorm::PreprocessTemplate() {
  ...
  musaMemsetAsync(pimpl->dev.CCG.get(), 0, sizeof(cufftComplex) * padded_template_size * batch_size,
                  pimpl->dev.stream);
